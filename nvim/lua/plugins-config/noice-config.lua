...
-- INFO: override `vim.ui` by using nui dependency from `noice.nvim`
local nui_input_status, Input = pcall(require, 'nui.input')
local nui_menu_status, Menu = pcall(require, 'nui.menu')
if nui_input_status and nui_menu_status then
    local event = require("nui.utils.autocmd").event

    local function get_prompt_text(prompt, default_prompt)
        local prompt_text = prompt or default_prompt
        local prompt_text_end = prompt_text:sub(-1)
        if  prompt_text_end == ":" then
            prompt_text = "[" .. prompt_text:sub(1, -2) .. "]"
        elseif prompt_text_end == " " then
            prompt_text = "[" .. prompt_text:sub(1, -3) .. "]"
        end
        return prompt_text
    end
    ...
    local function override_ui_input()
        ...
        function UIInput:init(opts, on_done)
            ...
            UIInput.super.init(self, {
                ...
                size = {
                    width = math.max(24, vim.api.nvim_strwidth(default_value)),
                },
                border = {
                    style = "rounded",
                    text = {
    ...
    -- INFO: calling overrided functions
    override_ui_input()
    override_ui_select()
end
